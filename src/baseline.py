...
def train_single_gpu(args):
    ...
    run_name = args.wandb_name or f"baseline_lora_r{args.lora_r}_bs{args.per_device_train_batch_size}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    summary_metrics = generate_summary_metrics(
        train_result, train_dataset, total_train_time, args, "single_gpu"
    )
